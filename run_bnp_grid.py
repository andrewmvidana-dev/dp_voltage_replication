...
def main():
    ...
    print(f"  voltage sensitivity S = {delta_V:.4f} pu  (sampled: a LOWER bound,")
    print(f"    and seed-unstable -- ~8.6x spread across seeds. See")
    print(f"    empirical_voltage_sensitivity(adversarial=True) for the honest figure.)")
    print(f"  output-stage bound    B = S/2 = {B_out:.4f} pu")
    print(f"    B = S/2 is the smallest bound Corollary 1 admits, and by")
    print(f"    delta = S/(2B) that is delta = {bnp_delta(delta_V, B_out):.1f} EXACTLY --")
    print(f"    NO privacy at all, not merely weak. It already exceeds the ANSI")
    print(f"    half-band of 0.05 pu by {B_out / 0.05:.1f}x.")
    ...
    print()
    print("  OUTPUT stage -- fails hardest, and fails before buying anything.")
    print("  B = S/2 = %.3f pu is the smallest bound Corollary 1 admits, and" % B_out)
    print("  delta = S/(2B) makes that delta = 1 EXACTLY: no privacy at all.")
    print("  Even there it exceeds the ANSI half-band of 0.05 by %.1fx, so most" % (B_out / 0.05))
    print("  released voltages leave the regulation band and the autocorrelation")
    print("  goes to zero. Figure 4 shows why no valid B exists.")
    print()
